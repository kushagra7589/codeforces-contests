...
void dfs(int current, int dest, vector<bool> &visited, vi &path, int &index, int T, ll max_time, vi &max_path, int &max_index)
{
	...
	else
	{
		for(auto i : graph[current])
		{
			if(visited[i.F] == false)
				dfs(i.F, dest, visited, path, index, T, max_time, max_path, max_index);
		}
	}
...
void dfs_visit(int s, int d, ll T)
{
	...
	vi max_path;
	int max_index = 0;
	dfs(s, d, visited, path, index, T, max_time, max_path, max_index);
	cout << max_index << endl; 
